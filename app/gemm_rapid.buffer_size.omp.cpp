...
//  for (uint64_t dim_size = 64; dim_size <= 128; dim_size *= 2) {
...
//  for (uint64_t tile_dim_size = 32; tile_dim_size <= 2048; tile_dim_size *= 2) {
  for (uint64_t tile_dim_size = 16; tile_dim_size <= 16; tile_dim_size *= 2) {
...
//                     C);
          gemm_v6_softcache_omp(A, A1, A2, A1_tile, A2_tile,
                            B, B1, B2, B1_tile, B2_tile,
                            C);
...
//    std::string tiling_filename = std::format("output.gemm.dram.tiling.size{}.log", dim_size);
    ...
    std::string collect_filename = "output.gemm.fam.collection.matrix-size-" + std::to_string(dim_size) + ".buffer-sizes.num-threads.csv";
    std::ofstream fout;
...
//    std::string header("Matrix_size,RAPID.No-Tiling(s),RAPID.SoftCache.Tiling(s)");
      ...
      for (uint64_t num_thd: num_threads) {
        header += ",RAPID.DoubleBuffer.Thread-" + std::to_string(num_thd);
      }
      fout << header << std::endl;
      for (uint64_t row_i = 0; row_i < tile_dim_sizes.size(); ++row_i) {
//      fout << dim_sizes[row_i] << "," << gemm_no_tiling_avg_times[row_i] << "," << gemm_tiling_avg_times[row_i] << std::endl;
        fout << tile_dim_sizes[row_i];
        for (uint64_t num_thd: num_threads) {
