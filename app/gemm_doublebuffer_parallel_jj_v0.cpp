...
//  for (uint64_t tile_dim_size = 16; tile_dim_size <= 64; tile_dim_size *= 2) {
    tile_dim_sizes.push_back(tile_dim_size);
  }

  std::vector<uint64_t> num_workers_list;
  for (uint64_t w = 1; w <= 16; w *= 2) {
    num_workers_list.push_back(w);  /// (2*w + 1 + 1) threads
  }
  num_workers_list.push_back(27);  /// (54 + 1 + 1) threads
//  for (uint64_t w = 8; w <= 16; w *= 2) {
//    num_workers_list.push_back(w);  /// (2*w + 1 + 1) threads
//  }
...
//        memset(C, 0, sizeof(double) * C1 * C2);
          ...
          gemm_v9_doublebuffer_parallel_jj(
