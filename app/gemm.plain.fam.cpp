...
//  for (uint64_t dim_size = 5; dim_size <= 5; dim_size *= 2) {
...
//    double *C = create_matrix_in_dram(C1, C2);
    ...
    for (uint64_t r_i = 0; r_i < num_repeats; ++r_i) {
      ...
      std::cout << "FAM plain dim_size: " << dim_size << ", r_i: " << r_i << ", time_exe(s): " << tt_duration.count() << "\n";
...
//      print_matrix(C, C1, C2);
    ...
    {
      std::stringstream ss;
      ss << "output.gemm.fam.plain.size" << dim_size << ".log";
...
//    destroy_matrix_in_dram(C);
  ...
  std::string collect_filename("output.gemm.fam.plain.collection.csv");
  std::ofstream fout;
  fout.open(collect_filename);
  if (fout.is_open()) {
    std::string header("Matrix_size,FAM.Plain(s)");
    fout << header << "\n";
