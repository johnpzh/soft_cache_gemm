...
func.func @main() {
  ...
  %compute_is_one = arith.cmpi eq, %num_compute_workers, %c1 : index
  %aux_is_one = arith.cmpi eq, %num_aux_workers, %c1 : index
  %both_are_one = arith.andi %compute_is_one, %aux_is_one : i1
  ...
  // scf.for %w = %c0 to %num_compute_workers step %c1 iter_args(%i_start = %c0) -> (index) {
    %A_buffer1_ptr_cast = memref.load %A_buffer1s[%c0] : memref<?xmemref<*xmemref<*xf64>>>
    %B_buffer1_ptr_cast = memref.load %B_buffer1s[%c0] : memref<?xmemref<*xmemref<*xf64>>>
    %A_buffer_is_ready_cast = memref.load %A_buffer_readys[%c0] : memref<?xmemref<*xi8>>
    %B_buffer_is_ready_cast = memref.load %B_buffer_readys[%c0] : memref<?xmemref<*xi8>>
    %num_local_i_tiles = memref.load %tiles_per_worker_list[%c0] : memref<?xindex>
    %A1_offset_ptr = memref.load %A1_offset_list[%c0] : memref<?xmemref<*xindex>>
    %A2_offset_ptr = memref.load %A2_offset_list[%c0] : memref<?xmemref<*xindex>>
    %A_block_rows_ptr = memref.load %A_block_rows_list[%c0] : memref<?xmemref<*xindex>>
    %A_block_cols_ptr = memref.load %A_block_cols_list[%c0] : memref<?xmemref<*xindex>>
    %B1_offset_ptr = memref.load %B1_offset_list[%c0] : memref<?xmemref<*xindex>>
    %B2_offset_ptr = memref.load %B2_offset_list[%c0] : memref<?xmemref<*xindex>>
    %B_block_rows_ptr = memref.load %B_block_rows_list[%c0] : memref<?xmemref<*xindex>>
    %B_block_cols_ptr = memref.load %B_block_cols_list[%c0] : memref<?xmemref<*xindex>>
    %is_finished_ptr = memref.load %compute_workers_finished[%c0] : memref<?xmemref<*xi8>>

    /// Call compute worker function asynchronously
    %token_compute = async.execute {
  ...
  // scf.for %w = %c0 to %num_aux_workers step %c1 iter_args(%compute_worker_start = %c0) -> (index) {
    %num_local_compute_workers = memref.load %compute_per_aux_list[%c0] : memref<?xindex>

    /// Call auxiliary worker function asynchronously
    %token_aux = async.execute {
      func.call @comet_double_buffer_aux_worker_pull(
        ...
        %c0,                           /// uint64_t compute_worker_start
  ...
  async.await %token_compute : !async.token
  async.await %token_aux : !async.token
